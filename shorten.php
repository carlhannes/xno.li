...
// Begin XNO.LI additions
if(empty($url_to_shorten)){
	die("Not a valid URL");
}

$urlparts = explode(".", $url_to_shorten);
if(count($urlparts)<2){
	die("Not a valid URL");
}

if(substr(strtolower($url_to_shorten), 0, strlen("http"))!="http"){
	$url_to_shorten = "http://".trim($url_to_shorten);
}

if(parse_url($url_to_shorten, PHP_URL_HOST) == $_SERVER['HTTP_HOST']){
	die($url_to_shorten);
	//die("Not a valid URL");
}
// End XNO.LI additions
...
if(!empty($url_to_shorten) && preg_match('|^https?://|', $url_to_shorten))
{
	...
	if(CHECK_DNS)
	{
		$dnsr = dns_get_record(parse_url($url_to_shorten, PHP_URL_HOST), DNS_A);
		if(!isset($dnsr[0]['ip'])){
			die('Not a valid URL');
		}
	}
